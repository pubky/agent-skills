...
function fromGit(spec) {
  ...
  return { body: readFileSync(file, 'utf8'), origin: `${spec.repo}:${spec.path}` }
}
...
function resolveBody(from) {
  const tried = []
  for (const spec of from) {
    const got = spec.endpoint ? fromEndpoint(spec) : fromGit(spec)
...
for (const [relPath, entry] of Object.entries(lock.vendoredSkills || {})) {
  const { body, origin, tried } = resolveBody(entry.from || [])
  if (!body) {
  ...
  changed++
  if (CHECK_ONLY) { console.error(`✗ ${relPath}: out of date vs ${origin}`); failed++; continue }
